...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    if (!emailRegex.test(email)) {
      return res.status(400).json({ errorMessage: "잘못된 이메일 형식입니다." });
    }

    //비밀번호 형식
    if (password.length < 6 || password !== confirmPassword) {
      return res.status(400).json({ errorMessage: "이메일또는 비밀번호 형식이 잘못되었습니다." });
    }

    //이미 등록된 이메일
    const existingUser = await models.User.findOne({ where: { email } });
    if (existingUser) {
      return res.status(400).json({ errorMessage: "이미 등록된 이메일 입니다." });
    }
