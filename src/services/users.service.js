...
export class UsersService {
  ...
  kakaoLogin = async (kakaoId, email, nickname) => {
    try {
      ...
      let userId;
      if (!user) {
        userId = await this.createKakaoUser(kakaoId, email, nickname);
      } else {
        userId = user.userId;
      }

      // Fetch the complete user data, including points
      const prismaUser = await this.usersRepository.findUserById(userId);

      // Return the user object with points
      return {
