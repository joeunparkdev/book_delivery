...
import { JWT_REFRESH_TOKEN_SECRET, JWT_ACCESS_TOKEN_SECRET, JWT_ACCESS_TOKEN_EXPIRES_IN } from "../constants/security.constant.js";
...
export class RefreshTokenService {
  ...
  async refreshAccessToken(user) {
    ...
    const refreshToken = await refreshTokenRepository.findTokenByUserId(user.id);

    // 만약 리프레시 토큰이 존재하고 유효하다면
    if (refreshToken && jwt.verify(refreshToken.token, JWT_REFRESH_TOKEN_SECRET)) {
      // 새로운 액세스 토큰 생성
      const newAccessToken = this.generateAccessToken(user.id);
      
      // 기존의 리프레시 토큰 삭제
      await refreshTokenRepository.deleteToken(user.id);
      ...
      await refreshTokenRepository.createToken(user.id, newRefreshToken, expirationDate);
  ...
  generateAccessToken(userId) {
    const accessTokenSecret = JWT_ACCESS_TOKEN_SECRET || "default_access_token_secret";
  ...
  generateRefreshToken(userId) {
    const refreshTokenSecret = JWT_REFRESH_TOKEN_SECRET || "default_refresh_token_secret";
